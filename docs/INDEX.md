...
- [Current Windows transport slice: Phase 15.7](PHASE15_7_WGC_BOUNDED_FRAME_TRANSPORT.md)
- `contracts/phase15/` contains observer view/nested Canvas, rendezvous, durable observer event, external observer command, Windows provider/window/native bridge contracts and `windows_capture_snapshot_v1`.
...
- `native/windows-bridge-csharp/` now covers Win32 discovery, HWND-bound WGC session lifecycle and bounded `png_base64_snapshot_v1` transport. UI Automation and Canvas portal rendering of those pixels remain later slices.
...
ADR-001 至 ADR-019 位於本目錄。ADR-013 至 ADR-015 固定 observer durability、authenticated re-entry 與 single-topology nested visibility；ADR-016 至 ADR-018 固定 Windows provider identity、native discovery 與 WGC session lifecycle；ADR-019 固定有界 latest-only PNG snapshot transport、跨 process integrity 驗證以及 frame transport 與 Canvas portal completion 的分離。Phase 13 是 Canvas-first 安全收斂與跨專案契約層；Phase 15 在其上加入多觀察者私有視圖、持久化、選擇性會合、遞歸 Canvas 導航與 Windows provider/native visual runtime。
