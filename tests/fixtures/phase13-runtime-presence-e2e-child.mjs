...
async function closeSocket(ws){if(!ws||ws.readyState===WebSocket.CLOSED)return;await new Promise(resolve=>{const timer=setTimeout(resolve,350);ws.addEventListener('close',()=>{clearTimeout(timer);resolve()},{once:true});try{ws.close()}catch{clearTimeout(timer);resolve()}})}
...
function runtime(overrides={}){return{provider:'herdr',providerResourceId:'terminal-claude',runtimeEpochId:'epoch-1',status:'working',revision:10,sequence:20,kind:'claude',focused:false,interactiveReady:true,launchPending:false,coordinates:{workspaceId:'herdr-ws',tabId:'herdr-tab',paneId:'herdr-pane'},...overrides}}
...
async function createRuntimeServer(){
  ...
  server.on('upgrade',(request,socket,head)=>{sockets.add(socket);socket.on('close',()=>sockets.delete(socket));hub.handleUpgrade(request,socket,head)})
  await new Promise((resolve,reject)=>{server.once('error',reject);server.listen(0,'127.0.0.1',resolve)})
  server.unref()
  const address=server.address();if(!address||typeof address==='string')throw new Error('missing test address')
  return {store,runtimePresence,url:`ws://127.0.0.1:${address.port}/sync`,destroy:()=>{for(const socket of sockets){try{socket.destroy()}catch{}};try{server.closeAllConnections?.()}catch{};try{server.close()}catch{}}}
}
...
async function scenarioBroadcastAndRemoval(){
  ...
  try{
    ...
    const stale=await staleRejected;assert.equal(stale.runtimePresence.status,'working')
    ...
    const restarted=await restartedSeen;assert.equal(restarted.runtimePresence.status,'idle');assert.equal(restarted.runtimePresence.revision,1)
    assert.equal(app.store.getCanvas('root').revision,revisionBefore)
    ...
    const removedState=await removed;assert.equal(removedState.runtimePresence.semanticAgentId,'agent:claude-main')
    assert.equal(app.runtimePresence.snapshot().length,0)
  }finally{await closeSocket(claude);await closeSocket(codex);app.destroy()}
}

async function scenarioHelloSnapshot(){
  const app=await createRuntimeServer();const claude=new WebSocket(app.url),codex=new WebSocket(app.url)
  try{
    await opened(claude);await hello(claude,{clientId:'bridge-claude',authToken:bindings[0].token})
    const own=message(claude,m=>m.type==='runtime_presence'&&m.runtimePresence?.status==='working')
    claude.send(JSON.stringify({type:'runtime_presence',runtime:runtime()}));await own
    await opened(codex)
    ...
    assert.equal(state.status,'working');assert.equal(state.semanticAgentId,'agent:claude-main')
  }finally{await closeSocket(claude);await closeSocket(codex);app.destroy()}
}
...
async function scenarioViewerRejected(){
  ...
  try{
    await opened(ws);await hello(ws,{clientId:'viewer-codex',authToken:bindings[1].token})
    ...
    const error=await rejected;assert.match(error.message,/viewer principal cannot publish runtime presence/)
    assert.equal(app.runtimePresence.snapshot().length,0);assert.equal(app.store.getCanvas('root').revision,0)
  }finally{await closeSocket(ws);app.destroy()}
}
