...
test('Windows provider schemas publish the bounded capture-session contract surface', async () => {
  ...
  assert.equal(capabilitySchema.properties.capture.properties.sessionLifecycleSupported.type, 'boolean')
  assert.deepEqual(capabilitySchema.properties.capture.properties.frameTransport.enum, ['none'])
...
test('global capability document claims WGC session lifecycle but not frame transport or UIA', () => {
  ...
  assert.equal(MRMIC_CAPABILITIES.windowsProvider.nativeBridge.scope, 'discovery_and_capture_session')
  ...
  assert.equal(MRMIC_CAPABILITIES.windowsProvider.nativeBridge.frameTransportImplemented, false)
  ...
  assert.equal(MRMIC_CAPABILITIES.windowsProvider.capture.frameTransport, 'none')
